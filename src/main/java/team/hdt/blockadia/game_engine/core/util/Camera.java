...
public class Camera {
    ...
    public static void init(float _aspect) {
        ...
        projection = Matrix4fs.createPerspectiveProjection(fov, aspect, 0.01f, z_far);
        view = Matrix4fs.createIdentityMatrix();

        position = new Vectors3f(0, 0, 0);
        rotation = new Vectors3f(0, 0, 0);

        mouse_locked = false;
        center = new Vectors2f(Window.width / 2, Window.height / 2);
...
//                Picker.insert(ray);
    ...
    public static void update() {
        ...
        Matrix4fs.rotate((float) Math.toRadians(rotation.x), Vectors3f.xAxis, view, view);
        Matrix4fs.rotate((float) Math.toRadians(rotation.y), Vectors3f.yAxis, view, view);
        Matrix4fs.rotate((float) Math.toRadians(rotation.z), Vectors3f.zAxis, view, view);

        Matrix4fs.translate(position, view, view);
    }

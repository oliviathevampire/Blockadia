package team.hdt.sandboxgame.game_engine.client.guis;
...
public abstract class GuiComponent {
	...
	public void addPixelComp(GuiComponent component, float relX, float relY){
		...
		float width = component.preferredPixelSize/(scale.x * Main.WIDTH);
		float height = component.preferredPixelSize/(scale.y * Main.HEIGHT);
	 ...
	 * @param relX
	 *            - the x position of the left edge of the text, relative to
	 *            this component's size and position (0 = far left edge, 1 = far
	 *            right).
	 * @param relY
	 *            - the y position of the top edge of the text, relative to this
	 *            component's size and position (0 = top edge, 1 = bottom edge).
	 * @param relLineWidth
	 *            - the width of the line of text, relative to the width of the
	 *            component.
	 */
	 ...
	 * 
	 * @param text
	 *            - the text currently in the component that needs to be
	 *            removed.
	 */
	...
	public float getPixelHeight(){
		return scale.y * Main.HEIGHT;
	}
	
	public float getPixelWidth(){
		return scale.x * Main.WIDTH;
	}
	...
	protected void setClippingBounds(float x, float y, float width, float height) {
		int xPixels = Math.round(x * Main.WIDTH);
		int yPixels = Main.HEIGHT - Math.round((y + height) * Main.HEIGHT);
		int widthPixels = Math.round(width * Main.WIDTH);
		int heightPixels = Math.round(height * Main.HEIGHT);
	...
	protected float pixelsToRelativeX(float pixels) {
		float pixelsWide = (float) Main.WIDTH * scale.x;
		return pixels / pixelsWide;
	}

	protected float pixelsToRelativeY(float pixels) {
		float pixelsHigh = (float) Main.HEIGHT * scale.y;
	 ...
	 * 
	 * @param data
	 *            - the list of {@link GuiTexture}s to be rendered.
	 */
	...
	protected float getRelativeHeightCoords(float relativeWidth) {
		relativeWidth *= ((float) Main.WIDTH / (float) Main.HEIGHT);
		relativeWidth *= scale.x / scale.y;
		return relativeWidth;
	}

	protected float getRelativeWidthCoords(float relativeHeight) {
		relativeHeight /= ((float) Main.WIDTH / (float) Main.HEIGHT);
	 ...
	 * 
	 * @param data
	 *            - the list of GUI textures that are going to be rendered. This
	 ...
	 * 
	 * @param text
	 *            - the text whose screen-space position needs to be set.
	 * @param relativePosition
	 *            - the position of the text relative to this component's

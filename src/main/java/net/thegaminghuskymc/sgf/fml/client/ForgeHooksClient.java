...
package net.thegaminghuskymc.sgf.fml.client;

import static net.thegaminghuskymc.sgf.common.ForgeVersion.Status.BETA;
import static net.thegaminghuskymc.sgf.common.ForgeVersion.Status.BETA_OUTDATED;
...
import net.thegaminghuskymc.sandboxgame.client.audio.ISound;
import net.thegaminghuskymc.sandboxgame.client.audio.SoundManager;
import net.thegaminghuskymc.sandboxgame.client.gui.FontRenderer;
import net.thegaminghuskymc.sandboxgame.client.renderer.RenderGlobal;
import net.thegaminghuskymc.sandboxgame.client.renderer.block.model.*;
import net.thegaminghuskymc.sandboxgame.client.renderer.texture.TextureAtlasSprite;
import net.thegaminghuskymc.sandboxgame.client.renderer.texture.TextureManager;
...
import net.thegaminghuskymc.sgf.common.ForgeVersion;
import net.thegaminghuskymc.sgf.common.model.IModelPart;
import net.thegaminghuskymc.sgf.common.model.ITransformation;
import net.thegaminghuskymc.sgf.common.model.TRSRTransformation;
import net.thegaminghuskymc.sandboxgame.GameEngine;
import net.thegaminghuskymc.sandboxgame.block.state.IBlockState;
import net.thegaminghuskymc.sandboxgame.client.renderer.BufferBuilder;
...
public class ForgeHooksClient
{
    ...
    {
        return GameEngine.EVENT_BUS.post(new DrawBlockHighlightEvent(context, player, target, subID, partialTicks));
    }

    public static void dispatchRenderLast(RenderGlobal context, float partialTicks)
    {
        GameEngine.EVENT_BUS.post(new RenderWorldLastEvent(context, partialTicks));
    }

    public static boolean renderFirstPersonHand(RenderGlobal context, float partialTicks, int renderPass)
    {
        return GameEngine.EVENT_BUS.post(new RenderHandEvent(context, partialTicks, renderPass));
    }
    ...
    {
        return GameEngine.EVENT_BUS.post(new RenderSpecificHandEvent(hand, partialTicks, interpPitch, swingProgress, equipProgress, stack));
    }

    public static void onTextureStitchedPre(TextureMap map)
    {
        GameEngine.EVENT_BUS.post(new TextureStitchEvent.Pre(map));
    ...
    {
        GameEngine.EVENT_BUS.post(new TextureStitchEvent.Post(map));
    }

    public static void onBlockColorsInit(BlockColors blockColors)
    {
        GameEngine.EVENT_BUS.post(new ColorHandlerEvent.Block(blockColors));
    }

    public static void onItemColorsInit(ItemColors itemColors, BlockColors blockColors)
    {
        GameEngine.EVENT_BUS.post(new ColorHandlerEvent.Item(itemColors, blockColors));
    }
    ...
    {
        return GameEngine.EVENT_BUS.post(new MouseEvent());
    }

    public static float getOffsetFOV(EntityPlayer entity, float fov)
    {
        FOVUpdateEvent fovUpdateEvent = new FOVUpdateEvent(entity, fov);
        GameEngine.EVENT_BUS.post(fovUpdateEvent);
    ...
    {
        if (!GameEngine.EVENT_BUS.post(new GuiScreenEvent.DrawScreenEvent.Pre(screen, mouseX, mouseY, partialTicks)))
            screen.drawScreen(mouseX, mouseY, partialTicks);
        GameEngine.EVENT_BUS.post(new GuiScreenEvent.DrawScreenEvent.Post(screen, mouseX, mouseY, partialTicks));
    }
    ...
    public static float getFogDensity(EntityRenderer renderer, Entity entity, IBlockState state, float partial, float density)
    {
        ...
        if (GameEngine.EVENT_BUS.post(event)) return event.getDensity();
        return -1;
    }
    ...
    {
        GameEngine.EVENT_BUS.post(new EntityViewRenderEvent.RenderFogEvent(renderer, entity, state, partial, mode, distance));
    }
    ...
    {
        GameEngine.EVENT_BUS.post(new ModelBakeEvent(modelManager, modelRegistry, modelLoader));
    ...
    public static Matrix4f getMatrix(ItemTransformVec3f transform)
    {
        ...
        m.setIdentity();
        m.translate(TRSRTransformation.toVecmath(transform.translation));
        t.setIdentity();
        t.rotateY(transform.rotation.y);
        m.mul(t);
        t.setIdentity();
        t.rotateX(transform.rotation.x);
        m.mul(t);
        t.setIdentity();
        t.rotateZ(transform.rotation.z);
        m.mul(t);
        t.setIdentity();
    ...
    {
        GameEngine.EVENT_BUS.post(new RenderGameOverlayEvent.Post(new RenderGameOverlayEvent(Animation.getPartialTickTime(), res), BOSSINFO));
    }
    ...
    {
        GameEngine.EVENT_BUS.post(new InputUpdateEvent(player, movementInput));
    }

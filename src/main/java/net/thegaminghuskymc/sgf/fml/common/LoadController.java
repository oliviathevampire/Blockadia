...
public class LoadController
{
    ...
    void disableMod(ModContainer mod)
    {
        ...
        if (errors.get(modId).isEmpty())
        {
            ...
            modStates.put(modId, LoaderState.ModState.DISABLED);
    ...
    public void buildModList(FMLLoadEvent event)
    {
        ...
        for (final ModContainer mod : loader.getModList())
        {
            ...
            EventBus bus = new EventBus((exception, context) -> LoadController.this.errorOccurred(mod, exception));
    ...
    public void propogateStateMessage(FMLEvent stateEvent)
    {
        ...
        ProgressManager.ProgressBar bar = ProgressManager.push(stateEvent.description(), activeModList.size(), true);
    ...
    private void sendEventToModContainer(FMLEvent stateEvent, ModContainer mc)
    {
        ...
        {
            if (av.getLabel()!= null && requirements.contains(av.getLabel()) && modStates.containsEntry(av.getLabel(),LoaderState.ModState.ERRORED))
            {
    ...
    public void printModStates(StringBuilder ret)
    {
        ...
        {
            table.add(
                modStates.get(mc.getModId()).stream().map(LoaderState.ModState::getMarker).reduce("", (a, b) -> a + b),
                mc.getModId(),
                mc.getVersion(),
    ...
    {
        return Iterables.getLast(modStates.get(selectedMod.getModId()), LoaderState.ModState.AVAILABLE);
    }

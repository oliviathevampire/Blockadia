...
public class Sphere {
     ...
     *
     * triangles : number of triangles vertices : number of verices floats :
     * number of floats
     **/
    ...
    private static void make_sphere(float[] data, int detail, float[] a, float[] b, float[] c) {
        if (detail == 0) {
            data[_index++] = a[0] * _rayon;
            data[_index++] = a[1] * _rayon;
            data[_index++] = a[2] * _rayon;
            data[_index++] = b[0] * _rayon;
            data[_index++] = b[1] * _rayon;
            data[_index++] = b[2] * _rayon;
            data[_index++] = c[0] * _rayon;
            data[_index++] = c[1] * _rayon;
            data[_index++] = c[2] * _rayon;
        } else {
            float ab[], ac[], bc[];
    ...
    public static float[] make(int detail, float rayon) {
        ...
        int indices[][] = {{4, 3, 0}, {1, 4, 0}, {3, 4, 5}, {4, 1, 5}, {0, 3, 2}, {0, 2, 1}, {5, 2, 3},
                {5, 1, 2}};

        float positions[][] = {{0, 0, -1}, {1, 0, 0}, {0, -1, 0}, {-1, 0, 0}, {0, 1, 0}, {0, 0, 1}};
        ...
        for (int i = 0; i < 8; i++) {
            make_sphere(data, detail, positions[indices[i][0]], positions[indices[i][1]], positions[indices[i][2]]);
        }

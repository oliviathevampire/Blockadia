...
public class CameraPerspectiveWorldEntity extends CameraPerspectiveWorldCentered {
    ...
    protected void calculateCameraPosition(float distance) {
        ...
        Vector3f dir = Vector3f.sub(this.getPosition(), pos, null);
    ...
    private void updateControls() {
        if (Objects.requireNonNull(GLH.glhGetWindow()).isKeyPressed(GLFW.GLFW_KEY_SPACE)) {
            this.getEntity().jump();
        }
        if (Objects.requireNonNull(GLH.glhGetWindow()).isKeyPressed(GLFW.GLFW_KEY_W)) {
            this.getEntity().addControl(Control.FORWARD);
        }
        if (Objects.requireNonNull(GLH.glhGetWindow()).isKeyPressed(GLFW.GLFW_KEY_S)) {
            this.getEntity().addControl(Control.BACKWARD);
        }
        if (Objects.requireNonNull(GLH.glhGetWindow()).isKeyPressed(GLFW.GLFW_KEY_D)) {
            this.getEntity().addControl(Control.STRAFE_RIGHT);
        }
        if (Objects.requireNonNull(GLH.glhGetWindow()).isKeyPressed(GLFW.GLFW_KEY_A)) {

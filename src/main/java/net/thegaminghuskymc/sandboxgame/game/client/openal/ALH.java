...
public class ALH {
	...
	public static void alhInit() {
		Logger.get().log(Logger.Level.FINE, "Initializing OpenAL.");
		...
		Logger.get().log(Logger.Level.FINE, "OpenAL Version: " + version);
		Logger.get().log(Logger.Level.FINE, "OpenAL Vendor: " + vendor);
		Logger.get().log(Logger.Level.FINE, "OpenAL Renderer: " + renderer);
	...
	public static void alhStop() {
		Logger.get().log(Logger.Level.FINE, "Stopping OpenAL");

		if (_objects == null) {
			Logger.get().log(Logger.Level.WARNING, "Tried to stop OpenAL, but wasnt started!");
			return;
		}
		...
		Logger.get().log(Logger.Level.FINE, "OpenAL stopped. " + _objects.size() + " object destroyed.");
	...
	public static int alhCheckError(String label) {
		...
		if (err != AL10.AL_NO_ERROR) {
			Logger.get().log(Logger.Level.WARNING, label + " : OpenAL error occured : " + AL10.alGetString(err));
		}
		return (err);
	}
	...
	static final String TEST_SOUND = "C:/Users/Bruker/AppData/Roaming/Husky's Sandbox Game/assets/Husky's Sandbox Game/sounds/pop.wav";
	...
	private static void testSourcePool() throws InterruptedException {
		...
		if (i != 5) {
			Logger.get().log(Logger.Level.ERROR, "SOMETHING WRONG HAPPENED ON 'testSourcePool()' test");
		}
